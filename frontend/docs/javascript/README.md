...
### Для чего используется оператор "!!"? (Для HR)
...
## Что такое неявное преобразование типов и в каких случаях оно встречается в Javascript? (Для HR)
...
### Какой тип имеют ключи у объекта в Javascript? (Джун)

Ответ: Ключи в объекте имеют строковый тип, `string`.


### Что выведут логи? (Джун)
...
### Что выведет консоль лог и почему? (Для HR)
...
#### Как сделать так, чтобы вывелось `{ Hello: 'Hello' }`? (Для HR)

Ответ:  `{ [msg]: msg }`, квадратные скобки возьмут значение переменной, а не ее название.



## Что такое деструктуризация? приведите примеры (Для HR)
...
### При деструктуризации идет копирование значения или ссылка на это значение? (мидл)
...
## Для чего используется спред оператор `...`? приведите пример (Для HR)
...
## Что такое ассинхронный код? (Для HR)
...
### Что делает `Promise.allSettled()`? (Мидл)
...
### Что выведет консоль лог и почему? (мидл)
